...
class glRenderer:
    ...
    def __init__(self, width=640, height=480, name='GL Renderer',
                 program_files=['renderer/shaders/simple140.fs', 'renderer/shaders/simple140.vs'], color_size=1,
                 ms_rate=1):
        self.width = width
        self.height = height
        self.name = name
        ...
        self.index_buffer = glGenBuffers(
            1)  # for Mesh face indices. Without this vertices should be repeated and ordered (3x times bigger)
    ...
    def drawBackgroundOrth(self):
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, self.data_texture.shape[1], self.data_texture.shape[0], 0, GL_BGR,
                     GL_UNSIGNED_BYTE, self.data_texture.data)
        texHeight, texWidth = self.data_texture.shape[:2]
        texHeight *= 0.5
        texWidth *= 0.5
    ...
    def display(self):
        ...
        gluPerspective(65, float(self.width) / float(self.height), 10,
                       6000)  # This should be called here (not in the reshpe)
        # glMatrixMode(GL_MODELVIEW)
        # Render the scene
...
def loadSMPL():
    ...
    beta = np.array(
        [-0.2723351, 0.24493244, 0.66188693, 3.080746, 1.803318, -0.06044015, -0.19139446, 0.07565568, 0.9439081,
         -0.51000655])
    ...
    densepose_info = sio.loadmat(
        '/home/hjoo/codes/glViewer/densepose_uv_data/UV_Processed.mat')  # All_FaceIndices (13774), All_Faces(13774), All_U(7829), All_U_norm(7829), All_V(7829), All_V_norm (7829), All_vertices (7829)
