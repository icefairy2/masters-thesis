...
class Visualizer(object):
   ...
   Visualizer to visuzlie SMPL reconstruction output from HMR family (HMR, SPIN, EFT)
    ...
    def _visualize_screenless_naive(self, meshList, skelList=None, body_bbox_list=None, img_original=None,
                                    show_side=False, vis=False, maxHeight=1080):
    ...
    def _visualize_gui_smplpose_basic(self, smpl, poseParamList, shapeParamList=None, colorList=None, isRotMat=False,
                                      scalingFactor=300, waittime=1):
        '''
        ...
        for i, poseParam in enumerate(poseParamList):
            ...
            if isRotMat:  # rot mat
                pred_output = smpl(betas=shapeParam, body_pose=poseParam_tensor[:, 1:],
                                   global_orient=poseParam_tensor[:, [0]], pose2rot=False)

            else:  # angle axis
                pred_output = smpl(betas=shapeParam, body_pose=poseParam_tensor[:, 3:],
                                   global_orient=poseParam_tensor[:, :3], pose2rot=True)
