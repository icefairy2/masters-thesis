    ...
    def drawMesh(self):
        ...
        if True:  # self.meshindex_data:
            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,
                         self.index_buffer)  # Note "GL_ELEMENT_ARRAY_BUFFER" instead of GL_ARRAY_BUFFER
            glBufferData(GL_ELEMENT_ARRAY_BUFFER, self.meshindex_data, GL_STATIC_DRAW)

        # glDrawArrays(GL_TRIANGLES, 0, self.n_vertices)
        glDrawElements(GL_TRIANGLES, len(self.meshindex_data), GL_UNSIGNED_INT,
                       None)  # For index array (mesh face data)
